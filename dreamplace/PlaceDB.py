...
import dreamplace
import process_def as process_def
...
""" % (
    ...
    def apply_force(self, params, node_x, node_y):
        """
        @brief apply placement solution and update database
        """
        # assign solution
        self.node_x[:self.num_physical_nodes] = node_x[:self.num_physical_nodes]
        self.node_y[:self.num_physical_nodes] = node_y[:self.num_physical_nodes]

        # unscale locations
        node_x, node_y = self.unscale_pl(params.shift_factor, params.scale_factor)

        # update raw database
        place_io.PlaceIOFunction.apply_force(self.rawdb, node_x, node_y)
