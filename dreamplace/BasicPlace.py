...
class PlaceDataCollection(object):
    ...
    def __init__(self, pos, params, placedb, device):
        ...
        with torch.no_grad():
            ...
            if self.target_density < 1 or params.macro_place_flag or params.remap_flag:
...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb, timer):
        ...
        if params.global_place_flag and params.random_center_init_flag:  # move to center of layout
            ...
            if params.remap_flag:
                self.init_pos[:placedb.num_movable_nodes][placedb.movable_macro_mask] = \
                    placedb.node_x[:placedb.num_movable_nodes][placedb.movable_macro_mask]
        ...
        if params.global_place_flag and params.random_center_init_flag:  # move to center of layout
            ...
            if params.remap_flag:
                self.init_pos[placedb.num_nodes:placedb.num_nodes +
                              placedb.num_movable_nodes][placedb.movable_macro_mask] = \
                                placedb.node_y[:placedb.num_movable_nodes][placedb.movable_macro_mask]
        ...
        if len(placedb.regions) > 0:
            ...
            params, placedb, self.data_collections, self.device)
        if params.macro_place_flag or params.remap_flag:
    ...
    def build_legalization(self, params, placedb, data_collections, device):
        ...
        def build_legalization_op(pos, ml_flag=True, gl_flag=True, al_flag=True):
            logging.info("Start legalization")
            pos1 = ml(pos, pos) if ml_flag else pos.data.clone()
            pos2 = gl(pos1, pos1) if gl_flag else pos1.data.clone()
            ...
            if not legal:
                ...
                return pos2
            pos3 = al(pos1, pos2) if al_flag else pos2.data.clone()
    ...
    def build_multi_fence_region_legalization(self, params, placedb, data_collections, device):
        ...
        def build_legalization_op(pos, ml_flag=True, gl_flag=True, al_flag=True):
            for i in range(len(placedb.regions)+1):
                pos, pos_ml, pos_gl = legal_ops[i][0](pos, ml_flag, gl_flag)
            ...
            if not legal:
                ...
                for i in range(len(placedb.regions)+1):
                    pos = legal_ops[i][1](pos, pos_ml_list[i], pos_gl_list[i], al_flag)
            return pos

        def build_individual_legalization_ops(pos, region_id, ml_flag=True, gl_flag=True):
            pos = legal_ops[region_id][0](pos, ml_flag, gl_flag)[0]
            return pos
    ...
    def build_fence_region_legalization(self, region_id, params, placedb, data_collections, device):
        ...
        def build_greedy_legalization_op(pos, ml_flag=True, gl_flag=True):
            ...
            logging.info("Start legalization")
            pos1 = ml(pos, pos) if ml_flag else pos.data.clone()
            result = gl(pos1, pos1) if gl_flag else pos1.data.clone()
        ...
        def build_abacus_legalization_op(pos_total, pos_ref, pos, al_flag=True):
            result = al(pos_ref, pos) if al_flag else pos.data.clone()
