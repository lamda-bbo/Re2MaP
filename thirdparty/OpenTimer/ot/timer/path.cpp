...
std::vector<Path> Timer::_report_timing(std::vector<Endpoint*>&& epts, size_t K) {
  ...
  // 打印Endpoint的长度
  std::cout << "Number of Endpoints: " << epts.size() << std::endl;
  assert(epts.size() <= K);
  ...
  _taskflow.transform_reduce(epts.begin(), epts.end(), heap,
    ...
    [&] (PathHeap heap, Endpoint* ept) {
      _spur(*ept, K, heap);
      // 打印heap的大小
      // std::cout << "0 Heap size after processing ept: " << heap.size() << std::endl;
      return heap;
    },
    [&] (Endpoint* ept) {
      PathHeap heap;
      _spur(*ept, K, heap);
      // 打印heap的大小
      // std::cout << "1 Heap size after processing ept: " << heap.size() << std::endl;
      return heap;
    }
  );
  tf::Executor _executor(16);
